...
Peer Review Feedback html docx

Final Report presentation schedule [gdoc]({{<var urls.final_presentation_schedule>}})

The final week of the project is dedicated to analyzing your data, figuring out how to report it, and wrapping up the class.  After that, you prepare and deliver a final report and presentation to describer your efforts.  This document describes the analysis week, the final report, and the final presentation.
...
**Required: Have a writing conference with an instructor** -- Josh write this part!
...
2. Sensor Design - for each sensor
    a. Expected stimulus sizes from theory
    b. Sensor Selection - what sensors did you select, why, how do they match expected behavior
    c. Circuit Design - calculations to show component selection and that signals will be in range
    d. Software Considerations - is the 10Hz `default_robot.ino` sampling rate fast enough, did you write any code for the sensor
    e. Sensor Validation/Calibration - show the sensor working in controlled conditions
3. Robot Design - mechanical calculations, sensor mounting, waterproofing, other circuits that haven't been discussed yet (eg: regulators)
4. Deployment Details - what steps were taken for each deployment, where were the deployments (and what were the trajectories), how did you account for safety
5. Results - include comparisons to theoretical models
6. Conclusion - recap, recommendations for future work, interpretation of results and main takeaway
...
A rough draft of your final report is due at 5:00PM on the last day of classes.

The final draft is due at at 11:59PM the day before your final presentation.