...
1. Find the Voltages the MPX5700 will produce when at depths between 0 m and 1 m. Recall that the sensor will experience atmospheric pressure at 0m. We will call this Voltage range the target Voltage range.
...
2. Design a Voltage divider that converts the thermistor resistance to Voltage.  The divider should use a supply Voltage of 5V, a resistor of your choice in the top position, and a thermistor in the bottom position.  Predict the output Voltages of this circuit when the thermistor is in a 0 °C bath to a 20 °C bath. Use appropriate measures of uncertainty.
3. Design an op-amp circuit that will shift and scale the output of the thermistor divider into the Teensy Voltage range (0.1V to 3.2V).  Use an MCP601 op-amp with a supply Voltage of 5V.
...
Another equation that relates thermistor resistance and temperature, the Steinhart-Hart Equation, is relevant here and is shown in @eq-steinhart-hart. In this equation, $T$ is the absolute temperature in Kelvin, $R$ is the resistance of the thermistor, and $A$, $B$, $C$ and $D$ are fitting coefficients called Steinhart-Hart coefficients.  Note that the $B$ variable in @eq-steinhart-hart is different than the $B$ variable in @eq-RB-thermistor.

$$ 1/T = A + B \ln R + C \ln^2 R + D \ln^3 R $$ {#eq-steinhart-hart}
...
3. Prepare a set of eight different temperature baths between 0 °C and 80 °C. 
...
We are going to round out our spread of temperature sensors by revisiting the  [MCP9701A](https://drive.google.com/open?id=1ZZ7An-GFcWQ5SsR5wrQlyf-qWexbjHdo) temperature sensor from E79.  This part is an example of an integrated linear circuit.  
