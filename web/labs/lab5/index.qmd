...
7. Capture some oscilloscope data and take an FFT of it using Matlab, then compare that FFT to the scope FFTs.  Do so by following these steps.
    ...
    3. Ensure the "max length" setting is selected on the "save" menu
    4. Save the waveform 
    ...
    7. Contrast the Matlab FFT against the scope provided FFT from items 6.2 and 6.3.
...
2. You won't have all the parameters for your analytical function (TX power, P-to-V conversion), but you can fit it to the data.
3. You need to be sure that your measurements capture evidence of multipath in the tank, so how closely will you need to space your measurements? **CLARIFY**
...
3. For each beacon, measure the received Voltage magnitude as you move your microphone away from the beacon.  As a reminder, the spacing of your measurements is very important for spotting an important real-world phenomenon called multipath, and you'll need to rely on your earlier calculations to figure out spacing.
4. Plot your analytical function on the same axes as your measured points.  
